...
- [The LIMIT/OFFSET story](#the-limitoffset-story)
- [Keyset pagination](#keyset-pagination)
- [When keyset wins, when it loses](#when-keyset-wins-when-it-loses)
- [Composite cursors](#composite-cursors)
- [Encoding the cursor](#encoding-the-cursor)
- [Total-count problem](#total-count-problem)
- [Index requirements for keyset](#index-requirements-for-keyset)
...
- **Approximate the total** from a cached or periodically recomputed estimate
- **Run COUNT separately**, accepting the extra cost:
  ```sql
  SELECT COUNT(*) AS total FROM hotel WHERE state = 'CA';
  ```
  With an index leading on `state`, this is a covered index scan rather than a document fetch — but it still walks every matching index entry, so its cost grows with the size of the result set, not the page.
...
If the index direction doesn't match, the planner injects an Order operator (an in-memory sort) and the keyset trick loses its constant-cost property.

Mixed directions in one ORDER BY (`rating DESC, name ASC`) need matching mixed directions in the index (`hotel(state, rating DESC, name)`) for the sort to be pushed down.
