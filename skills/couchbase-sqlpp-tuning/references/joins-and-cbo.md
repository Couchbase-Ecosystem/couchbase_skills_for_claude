...
**Edition gate up front:** hash joins and the cost-based optimizer are **Enterprise Edition only**. On Community Edition the planner considers only nested-loop joins, and a `USE HASH` hint is silently ignored — so on CE, join tuning is entirely about indexing the join key and driving from the selective side.

## Contents

- [ANSI JOIN — use this, not lookup JOIN](#ansi-join--use-this-not-lookup-join)
- [Join algorithms](#join-algorithms)
- [Driving from the most selective side](#driving-from-the-most-selective-side)
- [The join-side indexes you need](#the-join-side-indexes-you-need)
- [Reading a join plan](#reading-a-join-plan)
- [ORDERED hint](#ordered-hint)
- [USE_HASH / USE_NL](#use_hash--use_nl)
- [Subqueries vs joins](#subqueries-vs-joins)
- [Statistics for join queries](#statistics-for-join-queries)
...
-- Lookup JOIN — older form, more restrictive
SELECT u.name, o.total
FROM users u
JOIN orders o ON KEY o.user_id FOR u;
```

ANSI JOIN works on any indexed field. Lookup JOIN requires a key-side reference and is more limited. The documentation's own guidance is that because ANSI joins are standards-compliant and more flexible, you should "use ANSI JOIN and ANSI NEST exclusively, where possible." Lookup and index joins are not formally deprecated, so existing queries using them keep working — but write new ones as ANSI joins.
...
| `HashJoin` (**EE only**) | Both sides moderately sized; CBO has cardinality info | Build a hash table from the smaller side, probe with the larger. Wins when both sides are non-trivial. |
| `IndexJoin` (legacy) | Lookup-JOIN with key-only references | Rare in ANSI JOIN code. |

On Enterprise Edition, CBO picks between nested-loop and hash automatically when statistics are available; without statistics it falls back to nested-loop. One documented exception: for an ANSI join whose right-hand side is a subquery or a generic expression, the EE default method is hash. On Community Edition only nested-loop is ever considered.
...
CBO will reorder joins automatically if statistics are good. If statistics are missing or wrong, force the order with `/*+ ORDERED */`. Note this is the *only* query block hint — it takes no arguments and goes in the SELECT clause.
...
Two equivalent spellings. In the JOIN clause:

```sql
-- Force a hash join with orders as the probe side
SELECT u.name, o.total
FROM users u
...
Or as hint comments (7.6+), which is the preferred form in new code:

```sql
SELECT /*+ USE_HASH(o/PROBE) */ u.name, o.total
FROM users u INNER JOIN orders o ON o.user_id = u.id
WHERE u.status = 'admin';

SELECT /*+ USE_NL(o) */ u.name, o.total
FROM users u INNER JOIN orders o ON o.user_id = u.id
WHERE u.status = 'admin';
```

`/BUILD` names the build side, `/PROBE` the probe side; omit the suffix to let the optimizer choose from estimated cardinality. You can combine one join-method hint with one of `USE INDEX` or `USE KEYS`, but not two of the same kind.

Pick nested-loop when the outer side is small and the inner side has a tight index. Pick hash when both sides are non-trivial. On Community Edition the hash variants do nothing.
...
## Statistics for join queries

This is where CBO needs statistics most. On **7.6+** they are gathered automatically when an index is created or built, but they go stale as data changes. After bulk loads or significant data changes:
...
Then EXPLAIN the join query — `optimizer_estimates` should appear on the operators, and CBO can pick the algorithm and order.

On **8.0+ EE**, Auto Update Statistics can do this on a schedule instead, but it is off by default; check `system:aus` before assuming it is running. See `cost-based-optimizer.md`.

`UPDATE STATISTICS` is a write and it is Enterprise Edition only. **Get the user's explicit approval before running it** — it consumes Query and Index service resources, and the MCP server blocks it in its default read-only mode for exactly that reason.
