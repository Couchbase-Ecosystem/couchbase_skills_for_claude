...
- [What each boundary actually controls](#what-each-boundary-actually-controls)
- [The 3-question framework](#the-3-question-framework)
- [Common patterns](#common-patterns)
- [When NOT to use scopes](#when-not-to-use-scopes)
- [When NOT to use collections](#when-not-to-use-collections)
- [Migration: moving between boundaries](#migration-moving-between-boundaries)
...
- Has its own bucket type (Couchbase or Ephemeral) and, for Couchbase buckets, its own storage engine (Couchstore or Magma)
...
**You can create up to 30 buckets in a cluster** — that is the documented maximum. Well below that ceiling, per-bucket memory and management overhead starts to dominate, so plan for a handful of buckets in most clusters and use scopes and collections for finer separation.

Bucket types in current releases are **Couchbase** (memory + disk, replicated) and **Ephemeral** (memory only, no persistence). Couchbase buckets use one of two storage engines: **Couchstore**, suited to lower data volumes, or **Magma**, the high-performance engine for much larger volumes. **Magma is Enterprise Edition only**, and new buckets in 8.0 default to Magma with 128 vBuckets.
...
- Is the unit of SQL++ query context: with the query context set to scope X, `SELECT * FROM users` resolves to `bucket.X.users`
- Indexes can be scope-scoped (an index in scope A can't see data in scope B)

Default scope is `_default`. The documented ceiling is **up to 1000 scopes per cluster** — not per bucket — so a design that wants a scope per tenant needs to stay well inside that number across the whole cluster.
...
- Can have a per-collection `maxTTL`, so documents in collection A auto-expire and collection B's don't
- Can have its own change-history retention settings on Magma buckets (EE)
- Indexes are collection-scoped: a `CREATE INDEX` on collection A only indexes A's documents

Default collection is `_default`. The documented ceiling is **up to 1000 collections per cluster** — again, per cluster, not per scope or per bucket.
...
On TTL specifically, the precedence rules matter: a non-zero expiration set at a lower level wins over a higher level, so a document's own expiry overrides the collection's `maxTTL`, which overrides the bucket's — but a document expiry can never exceed the collection or bucket `maxTTL`. A collection `maxTTL` of `0` inherits the bucket's setting; `-1` explicitly opts out of bucket inheritance. See `time-series-and-ttl.md`.
...
With a 1000-scope-per-cluster ceiling, scope-per-tenant works for hundreds of tenants, not thousands. Beyond that, put the tenant ID in the key prefix and in a document field instead, and enforce isolation in the application and via RBAC on the collection.
...
**Alternative:** Bucket-per-tenant if tenants have wildly different sizes or need different SLAs. Cost: bucket overhead per tenant, and a hard ceiling of 30 buckets per cluster — so this works only for a small number of large tenants.
...
Scopes and collections were added in Couchbase 7.0. Pre-7.0 code and tools often don't understand them. If you're integrating with older clients, scopes can cause friction. For most modern setups this isn't an issue, but check your SDK versions.
...
- **Many buckets** — the cluster maximum is 30, and overhead bites well before that. Use scopes and collections for fine-grained separation
