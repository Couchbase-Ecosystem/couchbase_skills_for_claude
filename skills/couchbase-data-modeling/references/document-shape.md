...
- [The four-question test](#the-four-question-test)
- [Document size limits and targets](#document-size-limits-and-targets)
- [The "is this one document or many" test](#the-is-this-one-document-or-many-test)
- [Patterns by relationship cardinality](#patterns-by-relationship-cardinality)
...
- **Hard limit:** **20 MB** per item — Couchbase enforces this and the write fails above it
- **Practical target:** keep documents under ~1 MB
- **Sweet spot:** 1 KB to 100 KB

The 20 MB figure is the documented maximum value size. The 1 MB target is a heuristic, not a product limit — it's where the costs below start to dominate.

Why the practical limit matters:
- Every read pulls the entire document over the network, unless you use the Sub-Document API to read specific paths
...
Fetch all orders for user 42 with `WHERE META().id LIKE 'user_order::42::%'`, backed by a primary or `META().id` index. Simple, and it keeps the child documents addressable by a predictable key so you can also do a KV multi-get when you already know the IDs.
...
- **Documents that grow forever** (logs appended to a user doc, comments on a post embedded as an array) — split into separate documents addressed by composite keys or queried via index. Appending to an array is a read-modify-write of the whole document even when done through the Sub-Document API's array-append, because the document is still rewritten and re-replicated
