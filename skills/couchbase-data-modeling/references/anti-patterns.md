...
- [The "one big document" mistake](#the-one-big-document-mistake)
- [The "everything in `_default`" mistake](#the-everything-in-_default-mistake)
- [The "type field in the body, no key prefix" mistake](#the-type-field-in-the-body-no-key-prefix-mistake)
- [The "deeply nested array of arrays" mistake](#the-deeply-nested-array-of-arrays-mistake)
- [The "key contains mutable state" mistake](#the-key-contains-mutable-state-mistake)
- [The "timestamp as leading key component" mistake](#the-timestamp-as-leading-key-component-mistake)
- [The "embedded list that grows forever" mistake](#the-embedded-list-that-grows-forever-mistake)
- [The "denormalize without an update plan" mistake](#the-denormalize-without-an-update-plan-mistake)
- [The "no version field" mistake](#the-no-version-field-mistake)
- [The "primary index as a substitute for thinking" mistake](#the-primary-index-as-a-substitute-for-thinking-mistake)
- [The "no thought given to read-vs-write ratio" mistake](#the-no-thought-given-to-read-vs-write-ratio-mistake)
- [The "schema by accident" mistake](#the-schema-by-accident-mistake)
- [The "scope per user" mistake (B2C)](#the-scope-per-user-mistake-b2c)
- [The "no TTL on session data" mistake](#the-no-ttl-on-session-data-mistake)
- [The "ID is also the email" mistake](#the-id-is-also-the-email-mistake)
- [Quick "is this model OK?" checklist](#quick-is-this-model-ok-checklist)
...
**Fix:** Use named scopes and collections (Couchbase 7.0+). Per-domain collections at minimum: `users`, `orders`, `events`, and so on. See `boundaries.md`.
...
**Fix:** Create secondary indexes for the actual query patterns. `ADVISE <statement>` (Enterprise Edition) suggests candidates; the `couchbase-sqlpp-tuning` skill covers how to read and refine them.

Note for current releases: since **7.6**, dropping the primary index no longer guarantees unindexed queries fail loudly — an RBAC-controlled sequential scan can serve them instead. Check the plans, not just the index list.

**Spot it by:** a primary scan (or a sequential scan) in the EXPLAIN output, or the user describing query performance as "OK but it uses the primary index."
...
**Why it's wrong:** in most OLTP workloads reads substantially outnumber writes, so optimizing write-side tidiness at the cost of read amplification is usually backwards. Check the actual ratio before assuming it, though — ingest-heavy workloads invert it.
...
- Sample the collection's inferred schema periodically to find drift (the Couchbase MCP server's `get_schema_for_collection` tool, or `INFER` in SQL++)
...
**Why it's wrong:** there is a documented ceiling of **1000 scopes per cluster** (and 1000 collections per cluster). Hundreds of scopes are fine; millions are not possible, and thousands already exceed the limit.
...
**Fix:** set an expiry at write time via the SDK, or give the collection a `maxTTL` so no code path can forget. Beware two traps: a document expiry of `0` means "inherit", not "never expire", and a full replace resets the document's TTL unless the write preserves expiration. See `time-series-and-ttl.md`.
