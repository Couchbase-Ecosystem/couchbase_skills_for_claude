...
- [What a Couchbase key is and isn't](#what-a-couchbase-key-is-and-isnt)
- [Three key strategies](#three-key-strategies)
- [The hot-shard problem](#the-hot-shard-problem)
- [Key length tradeoffs](#key-length-tradeoffs)
- [Prefix conventions](#prefix-conventions)
- [Counters: when you need monotonically-increasing IDs](#counters-when-you-need-monotonically-increasing-ids)
...
A key is:
- A string up to **250 bytes** (the documented maximum document key length)
- Unique within a collection (NOT within a bucket — same key in different collections is fine)
- The fastest possible lookup path (a KV get by key is the cheapest read Couchbase offers, and needs no index)
...
**When this works:** you frequently want to scan a key range (`session::user_42::*`), and the separator lets you do that via a SQL++ `LIKE` predicate on `META().id`, or via a KV range scan.
...
In practice the hash still distributes these reasonably well, because the input strings differ. The real hot-shard problem appears with:
...
Key length has a real memory cost, because per-document metadata — including the key — is held in memory. The arithmetic is simple: the difference between a 50-byte key and a 200-byte key across a billion documents is 150 GB of key bytes alone, before any other metadata.

How much of that metadata is resident depends on the storage engine and eviction policy: Couchstore buckets keep metadata for all documents in memory, while Magma (**Enterprise Edition only**, and the default for new buckets in 8.0) is designed to work with much larger datasets without holding all metadata resident. Either way, shorter keys are cheaper; the magnitude of the win depends on your configuration, so size it with the `couchbase-sizing` skill rather than assuming.

| Key length | When acceptable |
|---|---|
| < 50 bytes | Default target. Most natural and composite keys fit here |
| 50-100 bytes | Fine for moderate document counts |
| 100-200 bytes | Justified by debuggability / readability requirements |
| > 200 bytes | Only if there's no alternative — the hard limit is 250 bytes |

These bands are a rule of thumb, not a documented threshold. The one hard number is the 250-byte limit; everything else is a memory/readability trade you should measure on your own data.
...
If the app needs `order::1`, `order::2`, `order::3`, ... use Couchbase's atomic counter operation. Every SDK exposes it on the binary/counter API (`binary().increment(...)` or equivalent), operating on a dedicated counter document:

```
increment("counter::orders", delta=1, initial=1)  ->  returns the new value
```

The increment is atomic across all writers — no read-modify-write race. Use the returned value as the new ID.

**Caveat:** the counter document is a single document, so every insert serialises through one vBucket. At high creation rates that becomes the bottleneck. Shard the counter into N counter documents and pick one at random per insert if you hit that wall — at the cost of losing a single global ordering.
...
- **Putting the entire document key into the document body verbatim**: redundant. The key IS the access path. Store it in the body only if you need to query by it in SQL++ using `META().id`
...
- **Need monotonically increasing IDs?** → the SDK's atomic counter increment on a dedicated counter document
