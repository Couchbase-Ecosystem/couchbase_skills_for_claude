...
- **The Array Indexing page contradicts itself on `UNNEST` and `DISTINCT ARRAY`, and the table is the wrong half.** [Table 1](https://docs.couchbase.com/server/current/n1ql/n1ql-language-reference/indexing-arrays.html) says `UNNEST` works with "only ALL, with array as leading index-key". Example 11 Query B on the same page is titled "UNNEST **not covered** when using the DISTINCT index" and prints a plan containing an `IndexScan3` on that DISTINCT index with `"exact": true` spans — used, but not covered. Example 9 Query C goes further and covers an `UNNEST` with a `DISTINCT` index by adding the whole array as a second key. The skills now teach the behaviour the examples demonstrate (usable, not coverable, fetch required, with the aggregation exception) rather than the table. **Worth an upstream documentation fix**, because the table as written sends people to rebuild indexes they did not need to rebuild. Identical text in 7.2, 7.6, 8.0 and Capella.
- **The DISTINCT vs ALL semantics are never defined in prose** anywhere in the SQL++ language reference — only the grammar is given. The meaning had to be taken from a 2017 forum answer by a Couchbase engineer. A one-paragraph definition on the Array Indexing page would close a real gap.
